...
transformed parameters {
  ...
  lambda = 0.5 + exp(mu_pr[5] + sigma[5] * lambda_pr);
}
...
model {
  ...
  for (j in 1:N) {
    ...
    for (k in 1:Tsubj[j]) {
      ...
      // calculation of p_burst in original hbayesdm v1.1.1 ewmv stan file: 
...
generated quantities {
  ...
  real<lower=0.5> mu_lambda = 0.5 + exp(mu_pr[5]);
